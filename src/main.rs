...
mod tests {
    ...
    fn test_verify_password_correct() {
        ...
        assert!(verify_password(&argon2, password, &parsed_hash));
    }
    ...
    fn test_verify_password_incorrect() {
        ...
        let salt = SaltString::generate(&mut rand::thread_rng());
        let password_hash_str = argon2.hash_password(correct_password.as_bytes(), &salt).unwrap().to_string();
        let parsed_hash = PasswordHash::new(&password_hash_str).unwrap();

        assert!(!verify_password(&argon2, incorrect_password, &parsed_hash));
    }
}
